    ...
    <section className="relative mx-auto flex min-h-screen max-w-3xl flex-col justify-center px-5 py-12 sm:px-8">
      {!started ? <article className="overflow-hidden rounded-3xl border border-white/10 bg-white/8 text-center shadow-2xl shadow-violet-950/40 backdrop-blur">
        ...
        <article className="rounded-3xl border border-white/10 bg-white/8 p-6 shadow-2xl shadow-violet-950/40 backdrop-blur sm:p-10">
          ...
          <div className="mt-10"><h2 className="text-xl font-bold sm:text-2xl">{question.text}</h2>{question.id === 'music' && <img className="mt-5 h-48 w-full rounded-2xl object-cover sm:h-60" src="./images/question-2.webp" alt="Музыка для сложного момента" />}{question.id === 'problem' && <img className="mt-5 h-48 w-full rounded-2xl object-cover sm:h-60" src="./images/question-3.webp" alt="Невыполнимая задача" />}{question.id === 'snack' && <img className="mt-5 h-48 w-full rounded-2xl object-cover sm:h-60" src="./images/question-4.webp" alt="Перекус для судьбоносного момента" />}<div className="mt-5 grid gap-3 sm:grid-cols-2">{question.options.map((option) => <button className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 text-left font-semibold text-slate-200 transition hover:border-violet-400 hover:bg-violet-500/15 focus:outline-none focus:ring-2 focus:ring-pink-400" key={option.text} onClick={() => chooseAnswer(option)} type="button">{option.text}</button>)}</div></div>
        </article>
      </>}
    </section>
  </main>
